...
export default function ReportsStatistics({ userRole, branchId }: ReportsStatisticsProps) {
  ...
  const [activeReportTab, setActiveReportTab] = useState<'finance' | 'cerebro'>('finance');
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* Tab Selectors */}
      <div className="flex border-b border-white/5 pb-1 gap-6 mb-6">
        <button
          onClick={() => setActiveReportTab('finance')}
          className={`pb-3 px-1 text-xs font-black uppercase tracking-widest transition-all cursor-pointer relative ${
            activeReportTab === 'finance'
              ? 'text-[#ffb700]'
              : 'text-text-muted hover:text-white'
          }`}
        >
          Métricas Financieras & EBITDA
          {activeReportTab === 'finance' && (
            <motion.div layoutId="reportsActiveLine" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#ffb700]" />
          )}
        </button>

        <button
          onClick={() => setActiveReportTab('cerebro')}
          className={`pb-3 px-1 text-xs font-black uppercase tracking-widest transition-all cursor-pointer flex items-center gap-2 relative ${
            activeReportTab === 'cerebro'
              ? 'text-[#ffb700]'
              : 'text-text-muted hover:text-white'
          }`}
        >
          <Brain className="w-4 h-4 animate-pulse text-[#ffb700]" />
          El Cerebro: Inteligencia AI & Pronósticos
          {activeReportTab === 'cerebro' && (
            <motion.div layoutId="reportsActiveLine" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#ffb700]" />
          )}
        </button>
      ...
      {activeReportTab === 'cerebro' ? (
        <CerebroAI userRole={userRole} branchId={branchId} />
      ) : (
        <>
